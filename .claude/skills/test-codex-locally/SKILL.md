...
- **Docker may not be running.** `docker compose` fails with `unix:///Users/andrew/.docker/run/docker.sock` if Desktop is stopped. `open -a Docker` and wait for `docker info` before step 1.
