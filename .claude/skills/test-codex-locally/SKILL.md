...
- **`codex exec` is one-shot.** A standalone `/force-model` call does not persist to the next `codex exec` (new session id every time, unless you `codex exec resume`). Put ` /force-model <model>` as the first line of the SAME prompt. The leading space is load-bearing — Codex consumes unknown slash tokens as local commands; a leading space makes it a normal user message the router can parse. Alternative: `-c 'model_providers.weave-local.http_headers."x-weave-force-model"="z-ai/glm-5.1"'` (headless equivalent; served on the same turn).
...
| `"x-weave-force-model" = "z-ai/glm-5.1"` | Pin a model without a slash command |
...
Headless pin, no slash command:

```bash
CODEX_HOME=/tmp/weave-codex-local \
  codex exec --skip-git-repo-check -s read-only -C "$(pwd)" \
  -c 'model_providers.weave-local.http_headers."x-weave-force-model"="gpt-5.6-terra"' \
  "say hi and stop"
```
...
If you only see other models, `/force-model` didn't take — recheck the leading space in step 5, or use `x-weave-force-model`.
