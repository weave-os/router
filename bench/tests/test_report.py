...
def _analytics_row(session: str, model: str, cost: float) -> dict[str, object]:
    return {
...
def test_pairing_statistics_and_error_categories(two_arm_job: dict[str, Path]) -> None:
    ...
    assert sol.agent_cost_usd == pytest.approx(6.0)
...
def test_analytics_rows_join_on_codex_session(two_arm_job: dict[str, Path]) -> None:
    rows = [
        ...
        _analytics_row("router-t1", "claude-haiku-4-5", 0.01),
    ...
    assert router.router_list_price_usd is not None and router.router_list_price_usd > 0
    assert sol.router_billed_usd is None
    (pair,) = report.pairs
    assert pair.cost_ratio == pytest.approx(0.51 / 6.0)
