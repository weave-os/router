...
if with_timeout 30 env PI_CODING_AGENT_DIR="$PI_DIR" \
  ...
  [ "$(grep -Ec '^(✔ |ok [0-9]+ - )' "$WORK/unit.out" || true)" = "38" ] \
    && ok "pricing, beta, force-model, UI, compaction, and served-window unit suite passed" \
    || bad "unit suite did not report all 38 passes (see $WORK/unit.out)"
else
