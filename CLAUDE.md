...
- **Provider/model compatibility is Go-owned.** Policy sidecars may extract generic features and classify complexity, but must never recognize native tool request shapes or use provider/model identity to enforce compatibility, eligibility, pass-through, pins, fallbacks, or dispatch. Put native search detection in `internal/websearch`, semantic requirements in `internal/translate`, and routing enforcement in `internal/proxy` / `internal/router`. The Go router must filter candidates before a sidecar call or bypass the sidecar entirely.
