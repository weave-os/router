...
| `ROUTER_ESCALATION_JUDGE_ENABLED` | `false` | Enables the optional Weave-funded Switchyard LLM judge and requires `ROUTER_ESCALATION_JUDGE_API_KEY`. The judge uses only the OpenRouter binding for `deepseek/deepseek-v4-flash`. |
| `ROUTER_ESCALATION_JUDGE_API_KEY` | *(none)* | Dedicated OpenRouter credential for escalation judgments. Customer credentials, custom gateways, and subscription tokens are never used. |
