...
| Variable                          | Default                      | Purpose |
| --------------------------------- | ---------------------------- | ------- |
| `ROUTER_DEPENDENCY_FAIL_OPEN` | `false` | On internal preparation failure, relay one independently authorized request to the originally requested model. Preserve hard restrictions, native payloads and client cancellation. Uses a 12s preparation budget, 1s aggregate DB budget, 250ms DB calls and a 5s dependency cooldown. Authentication, subscription enrollment and the spend gates may serve a 30s last-known-good copy of a read that already succeeded in this process; an unknown key, a cold cache or an explicitly invalidated installation still fails closed. When enabled, readiness can report serving while PostgreSQL, Pub/Sub invalidation, the cluster scorer or HMM sidecar is degraded; the readiness response and startup logs identify the degraded mode. |
