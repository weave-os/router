...
| `ROUTER_COMPACTION_MODEL`         | `claude-sonnet-4-6`          | Model the compaction cascade summarizes with (and Claude Code's native compaction turn is pinned to) when the session has no warm Anthropic pin to reuse. Must have a binding on `ROUTER_HANDOVER_PROVIDER` and be one of the compaction policies' fixed catalog models in [`POLICY_INFERENCE.md`](POLICY_INFERENCE.md); `claude-fable-5` is used automatically for histories that exceed its window. |
