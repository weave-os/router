...
| `ROUTER_HMM_SIDECAR_TIMEOUT_MS`    | `4500`  | Total HMM decision timeout, shared across retries and reroutes within one request. |
| `ROUTER_POLICY_DEADLINE_FALLBACK` | `true` | Enable independently authorized serving recovery for HMM dependency failures. The legacy variable name now covers evidence, transport, timeout, authentication and contract failures. See [serving recovery](POLICY_RECOVERY.md). |
| `ROUTER_POLICY_DEADLINE_DEFAULT_MODEL` | *(none)* | Preferred local recovery model after valid successful session history. Normal eligibility checks apply; other eligible local models remain available if this preference is unavailable. |
| `ROUTER_HMM_SIDECAR_ATTEMPT_TIMEOUT_MS` | 40% of the decision timeout, floored at 1800ms when the budget can still retry | Bounds a single HMM attempt so one stalled sidecar instance cannot spend the whole decision budget before the retries run. Set it equal to `ROUTER_HMM_SIDECAR_TIMEOUT_MS`, or to `0`, to let one attempt use the full budget. |
...
| `ROUTER_HMM_ROSTER_PATH` | *(none)* | Generated declarative roster validated against the catalog for Go-owned arm selection. Missing or invalid configuration permits degraded startup only when local serving recovery is enabled and has a valid target; otherwise startup fails. Failed classification never supplies recovery candidates. Explicit controls retain their strict contracts. |
