...
| `ROUTER_HMM_ROSTER_PATH`           | *(none)* | Path to a generated declarative roster JSON (`hmm_router_cluster_roster_v6`). When set, the roster is loaded and validated against the model catalog at startup (boot fails on any invalid arm) and a summary is logged. Nothing serves from it; its only consumer is the log-only selection shadow. |
