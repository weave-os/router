...
3,000 UTF-8 bytes per text and 48,000 text bytes total. Original turn counts,
...
sidecar gate. `TestBundledPythonAcceptsGoClassificationEvidence` invokes the
actual bundled Python feature builder against Go-produced wire requests.
Set `HMM_CONTRACT_PYTHON` to the sidecar environment's Python. Optional
`HMM_CONTRACT_EXPORT` writes synthetic wire fixtures for companion validation.
...
its release, validate the exported fixtures with that exact revision's input
