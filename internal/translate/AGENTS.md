...
**The envelope carries the minting upstream's scope, and a replay requires an exact match** (`EmitOptions.ReasoningReplayScope`, fingerprinted by `proxy.reasoningReplayScope` over provider + wire model + base URL + hashed credential). Encrypted reasoning is decryptable only by the account and model that produced it: replayed elsewhere, Cortex answers `encrypted reasoning was created for a different account or model` and xAI `Encrypted content could not be decrypted or parsed` — a 400 that kills the turn rather than degrading it, and with it the client session (prod 2026-09, Cortex Grok/GLM turns re-routed mid-session). An envelope minted for another scope, or a turn whose scope is unknown, drops the reasoning item and sends the rest of the history. This is why the scope must be a *dispatch-target* fingerprint, not a model name: the same model served through a second Cortex account cannot decrypt the first account's reasoning either.
