...
2. **Implement `Proxy` and `Passthrough`.** The adapter translates the prepared request body to the provider's wire format, sends it with a pooled `http.Client` (build it with `httputil.NewClient(httputil.NewTransport(...))` so redirects are refused and the managed-mode destination policy applies, and use `httputil.StreamBody`), streams the response back. Handle the refused 3xx before the `>= 400` branch: `httputil.IsRedirect` + `httputil.RefusedRedirectError` on the Proxy path (buffered, retryable 502) and `httputil.WriteRefusedRedirect` on the Passthrough path — never relay the 3xx/Location, or the client follows it with its own key. Adapters call `proxy.OnUpstreamMeta` when they observe usage/header data. Do not leak provider-specific types across the package boundary.
