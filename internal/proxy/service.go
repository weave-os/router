...
func (s *Service) ProxyMessages(ctx context.Context, body []byte, w http.ResponseWriter, r *http.Request) (returnErr error) {
	...
	// rescue that swallowed the flush) still owe the diagnostics, and the
	// demotion below reads the snapshot.
	if preludeBuf.Committed() {
		streamCut.noteCut(proxyErr)
	}
	...
	if !agentShadowMode && !routeRes.BlindExperimentPassthrough {
		...
		// fail over; the next turn must not re-pick the same arm, unless the
		// client is about to re-send this one.
		armDemoted = s.maybeDemoteArmAfterCommittedStreamFailure(ctx, preludeBuf.Committed(), streamCut.clientRetriesCut(), routeRes.HardPinned, proxyErr, decision.Model, decision.Reason, installationID, routeRes.SessionKey, stickyStateRole(routeRes), routeRes.PinRole)
...
func (s *Service) ProxyOpenAIChatCompletion(ctx context.Context, body []byte, w http.ResponseWriter, r *http.Request) (returnErr error) {
	...
	*r = *r.WithContext(ctx)
	...
	if !routeRes.BlindExperimentPassthrough {
		...
		// demotion rationale.
		armDemotedOAI = s.maybeDemoteArmAfterCommittedStreamFailure(ctx, committed(preludeBuf) || committed(responsesPreludeBuf), false, routeRes.HardPinned, proxyErr, decision.Model, decision.Reason, installationIDFromContext(ctx), routeRes.SessionKey, stickyStateRole(routeRes), routeRes.PinRole)
