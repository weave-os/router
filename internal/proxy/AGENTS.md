...
`ProxyMessages` / `ProxyOpenAIChatCompletion` / `ProxyGeminiGenerateContent` call [`maybeCompact`](compaction.go) before routing. At `ROUTER_COMPACTION_PCT` (default 0.85) of the largest eligible context window, the cascade clears old tool results, then attempts a structured Anthropic summary. The summary request is measured after translation and tool-definition removal; histories too large for one request are summarized in bounded, tool-pair-aligned chunks. It returns `ErrContextWindowExceeded` (HTTP 413) with the original history intact if no safe summary fits. It never silently replaces history with a fixed tail. Summary inference uses the independent policy-reviewed `precompaction_summary` purpose even when main-turn model selection is authoritative; the tenant's provider and model restrictions still apply. Summary inference is billed separately as `_precompaction_summary`.

Summary selection tries the active Anthropic session family, `ROUTER_COMPACTION_MODEL` (default `claude-sonnet-5`), then the large-window `claude-opus-5` family. Each resolves to its newest eligible catalog version using numeric family/version comparison; model variants stay separate. Selection honors context fit and model exclusions. Auxiliary summaries use their dedicated Anthropic client independently of the ordinary routing pool. Claude Code 2.1.282 and later own their history even when the pool is smaller than the client budget: a fitting request passes through and an overflow returns HTTP 413 with a `prompt is too long` message to trigger native recovery. Its multi-turn 413 recovery was verified against a local Messages stub. Earlier versions defer only while a verified per-request harness budget fits the serving pool; an overflow or an unverified version uses the router cascade. Other harnesses use the router cascade. Native Responses bytes are not rewritten.
