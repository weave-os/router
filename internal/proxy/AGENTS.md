...
Each lane admits at most 256 outstanding jobs and 16 MiB of serialized payload (including running jobs). There is no per-job size ceiling and no producer-side size estimate: every input is already bounded at its source (capped response capture, request limits, metadata-sized rows). A job larger than the lane budget waits for an empty lane and is admitted alone, so retained bytes stay bounded at max(budget, one job). Blocked producers retain their input until admission; the queue's byte counter is not a measurement of total request or process memory. Fixed concurrency is one DB worker and two remote workers. Reports retain their two-second execution deadlines; attempts and request telemetry get five seconds, starting when a worker executes the job rather than when it was enqueued. There are no automatic retries.

`router.observations.dropped` reports only serialization failures (`invalid_payload`), shutdown rejections (`closing`), execution failures and panics, using lane/kind/reason only; capacity, size and backlog age never cause drops. Correlated error logs are limited to one per lane per ten seconds. Shutdown wakes and rejects waiting producers, then drains admitted work within the process deadline. I/O errors and termination still do not provide durable delivery guarantees. Jobs must not submit back into their own full queue.
