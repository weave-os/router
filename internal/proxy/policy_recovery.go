...
func (s *Service) recoverPolicy(ctx context.Context, req router.Request, failure error) (router.Decision, error) {
	...
	recovery := &router.ServingRecovery{Plans: authorizedPlans, Budget: &inference.AttemptBudget{Remaining: policy.RecoveryMaxAttempts, Deadline: time.Now().Add(policy.RecoveryRetryWindow)}, ClassifierFailure: failure}
...
func recordRecoveryOutcome(ctx context.Context, decision router.Decision, err error) {
	...
	apm.RecordPolicyRecovery(ctx, policy.FailureReasonFor(decision.Recovery.ClassifierFailure), outcome)
}
