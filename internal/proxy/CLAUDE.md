...
`WithObservationWorkers` injects the process-owned DB/remote lanes; nil disables observation persistence/reporting. With available capacity, attempt telemetry does not wait for persistence before the next allowed attempt. At saturation, the submitting caller waits for space: there is no capacity-based load shedding or caller-runs bypass of the worker limits. Request telemetry and outcome/training-feedback reports submit serialized snapshots, preserving IDs, timestamps, usage and consent. Oversize jobs are rejected as a whole, never truncated into misleading training samples. The durable `/rf` save is not queued.

Each lane admits at most 256 outstanding jobs and 16 MiB of serialized payload (including running jobs), with a 1 MiB per-job ceiling. There is no queue-age expiry. Producers precheck variable-size inputs before serialization. Decoded values and sink serialization are additionally bounded by fixed concurrency (one DB, two remote) and the per-job ceiling. Blocked producers retain their input until admission; the queue's byte counter is not a measurement of total request or process memory. Reports retain their two-second execution deadlines; attempts and request telemetry get five seconds, starting when a worker executes the job rather than when it was enqueued. There are no automatic retries.

`router.observations.dropped` reports explicit admission rejections, execution failures and panics using lane/kind/reason only; capacity pressure and backlog age never cause drops. Correlated error logs are limited to one per lane per ten seconds. Shutdown wakes and rejects waiting producers, then drains admitted work within the process deadline. I/O errors and termination still do not provide durable delivery guarantees. Jobs must not submit back into their own full queue.
