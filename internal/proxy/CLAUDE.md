...
Summary selection tries the active Anthropic session family, `ROUTER_COMPACTION_MODEL` (default `claude-sonnet-5`), then the large-window `claude-opus-5` family. Each resolves to its newest eligible catalog version using numeric family/version comparison; model variants stay separate. Selection honors context fit and model exclusions. Auxiliary summaries use their dedicated Anthropic client independently of the ordinary routing pool. Claude Code owns any request that fits the serving pool: it passes through untouched on every version, so the reported usage stays real and the client auto-compacts itself. On overflow, Claude Code 2.1.282 and later get HTTP 413 with a `prompt is too long` message to trigger native recovery (multi-turn recovery verified against a local Messages stub); earlier versions use the router cascade. Other harnesses use the router cascade. Native Responses bytes are not rewritten.
