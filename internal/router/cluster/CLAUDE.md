...
- **Don't resolve a request's providers through catalog bindings when `req.GatewayProviders` is non-empty.** A gateway-exclusive installation routes only what its keys alias (`RequestBindings.resolve`); a catalog `openai_gateway`/`anthropic_gateway` binding would otherwise ship an unaliased model name to an endpoint that doesn't serve it. An empty pool there is `policy.ErrGatewayServesNoDeployedModel` (400), not `ErrClusterUnavailable`.
