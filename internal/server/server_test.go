...
func TestRegister_DeploymentMode(t *testing.T) {
	...
	dashboardRoutes := []string{
		...
		"PUT /admin/v1/model-status",
		"GET /admin/v1/provider-inventory",
	}
	...
	t.Run("selfhosted mounts dashboard and product routes", func(t *testing.T) {
		...
		statusStore := modelstatus.New(time.Now, time.Minute, 5*time.Minute, nil)
		server.Register(engine, nil, nil, fakeDeployedModelsSource{}, nil, server.DeploymentModeSelfHosted, nil, nil, nil, nil, nil, statusStore)
		got := routeSet(engine)
	...
	t.Run("managed skips dashboard but keeps product routes", func(t *testing.T) {
		...
		server.Register(engine, nil, nil, fakeDeployedModelsSource{}, nil, server.DeploymentModeManaged, nil, nil, nil, nil, nil, nil)
		got := routeSet(engine)
		for _, want := range productRoutes {
	...
	t.Run("nil deployed-models source skips catalog endpoint", func(t *testing.T) {
		engine := gin.New()
		server.Register(engine, nil, nil, nil, nil, server.DeploymentModeManaged, nil, nil, nil, nil, nil, nil)
		got := routeSet(engine)
...
func TestRegisterMountsAnalyticsExportInBothModes(t *testing.T) {
	...
	for _, mode := range []server.DeploymentMode{server.DeploymentModeSelfHosted, server.DeploymentModeManaged} {
		t.Run(string(mode), func(t *testing.T) {
			engine := gin.New()
			server.Register(engine, nil, nil, nil, nil, mode, nil, nil, nil, analytics.NewService(nil, nil), nil, nil)
			got := routeSet(engine)
	...
	t.Run("nil service leaves the surface unmounted", func(t *testing.T) {
		engine := gin.New()
		server.Register(engine, nil, nil, nil, nil, server.DeploymentModeSelfHosted, nil, nil, nil, nil, nil, nil)
		got := routeSet(engine)
...
func TestRegisterSeparatesLivenessFromReadiness(t *testing.T) {
	...
	server.Register(engine, nil, nil, nil, nil, server.DeploymentModeManaged, nil, checker, nil, nil, nil, nil)
