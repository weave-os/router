...
func TestServingLifecycleRequiresExactProposalBeforeOpeningRegistry(t *testing.T) {
	err := run(context.Background(), []string{string(commandServing), string(commandApply)})
...
func TestServingRemovedVerbsExitNonZeroThroughTheRootCommand(t *testing.T) {
	for verb, replacement := range map[string]string{"validate": "publish --dry-run", "resolve": "apply --proposal-sha256", "prepare": "apply --dry-run", "activate": "serving apply", "rollback": "apply --proposal <ref> (proposal scope must be rollback)"} {
